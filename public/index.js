...
function loadWishList()
{
 ...
 $.ajax({
    ...
    success: function(data){
      ...
      //console.log("Current wishlist is " + JSON.stringify(data.wishlist));
    }
  });
}
  ...
  <li data-id="${game.id}" class="gameCard" >
    <div class="cardWrapper">
       ...
       <img src="${game.image.small_url}" alt="This is a clickable picture that will take you to the game" class="cardImage" title="Related Game Image"> 
        <span class="gameID">Expected Release Date: ${topDate}</span>
      <div>
        <button id="wishButton">+ Add to Wishlist</button><a href="${BASE_URL}${game.name}/${baseID}${game.id}/" target="_blank"><button id="infoButton"> ≡ More Info</button></a>
      </div>
...
function generateGamesHTML(game){
  ...
  <li data-id="${game.results.id}" class="gameCard" >
    <div class="cardWrapper">
      ...
      <div>
        <button id="wishButton">+ Add to Wishlist</button><a href="${BASE_URL}${game.results.name}/${baseID}${game.results.id}/" target="_blank"><button id="infoButton"> ≡ More Info</button></a>
      </div>
      </a>
    </div>
  </li>`;
